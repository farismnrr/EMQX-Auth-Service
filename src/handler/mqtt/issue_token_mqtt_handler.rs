...
use crate::dtos::mqtt_admin_dto::{MqttRpcRequest, MqttRpcRequestEnvelope, RpcIssueTokenData};
...
impl IssueTokenHandler {
    ...
    pub async fn issue_token_mqtt_handler(&self, payload: &str) {
        debug!("🎫 Handling RPC issue token request");
        ...
        // NOTE: Authorization is enforced via
        // broker ACLs and allowed requesters list.
        ...
        let request: MqttRpcRequest<RpcIssueTokenData> = match serde_json::from_str(payload) {
            Ok(req) => req,
            Err(e) => {
                error!("❌ Failed to parse RPC issue token request: {}", e);
                let error_response = error_response::<()>(
                    request_id,
        ...
        let response = self
            .admin_service
            .rpc_issue_token(request.request_id().to_string(), request.data)
            .await;
