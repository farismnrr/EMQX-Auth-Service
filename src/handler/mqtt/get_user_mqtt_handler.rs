...
use crate::dtos::mqtt_admin_dto::{MqttRpcRequest, MqttRpcRequestEnvelope, RpcGetUserData};
...
impl GetUserHandler {
    ...
    pub async fn get_user_mqtt_handler(&self, payload: &str) {
        debug!("🔍 Handling RPC get user request");
        ...
        // Security validation (no target_user match required for get)
        ...
        let request: MqttRpcRequest<RpcGetUserData> = match serde_json::from_str(payload) {
            Ok(req) => req,
            Err(e) => {
                error!("❌ Failed to parse RPC get user request: {}", e);
                let error_response = error_response::<()>(
                    request_id,
        ...
        let response = self
            .admin_service
            .rpc_get_user(request.request_id().to_string(), request.data)
            .await;
